...
func get_car_data_as_utf8_JSON():
	print($Car.rotation)
	var data = [$Car.car_position[0],$Car.car_position[1],$Car.velocity.length()/32*3.6,rad2deg($Car.steer_direction),$Car.score,$Car.distance_sensors2,$Car.parkpos,$Car.direction_degree]
